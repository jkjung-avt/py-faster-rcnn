...
def parse_rec(annoations):
    objects = []    
    for anno in annoations:
        if 'width' in anno:
            obj_struct = {}
            obj_struct['bbox'] = [ int(anno['x']),
                                   int(anno['y']),
                                   int(anno['x'] + anno['width']),
                                   int(anno['y'] + anno['height'])]
            obj_struct['name'] = anno['class']
            obj_struct['difficult'] = 0
            objects.append(obj_struct)
    return objects

def vehicles_ap(rec, prec, use_07_metric=False):
    """ ap = vehicles_ap(rec, prec, [use_07_metric])
